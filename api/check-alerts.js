...
//
// Cada entrada de alerts:{installationId} puede tener hasta tres modos
// activos a la vez para el mismo producto: targetPrice (bajada), riseTargetPrice
// (suba, Premium) y anyChange (cualquier cambio, Premium). Se chequean todos
// en la misma pasada para no pedir el precio a la API de ML mas de una vez
// por producto.
        ...
        for (const installationId of installationIds) {
                  try {
                    ...
                    for (const alert of alerts) {
                                  const dropPending = !!alert.targetPrice && !alert.triggered;
                                  const risePending = !!alert.riseTargetPrice && !alert.riseTriggered;
                                  const anyChangePending = !!alert.anyChange;

                                if (!dropPending && !risePending && !anyChangePending) {
                                ...
                                // Alerta de "cualquier cambio de precio" (Premium). No se marca
                                // triggered: sigue viva monitoreando indefinidamente hasta que el
                                // usuario la saca desde la extension. No se combina con drop/rise
                                // en la misma pasada.
                                if (anyChangePending) {
                                ...
                                // Alerta de precio objetivo, bajada (clasica)
                                if (dropPending && currentPrice <= alert.targetPrice) {
                                                stats.triggered++;
                                ...
                                // Alerta de precio objetivo, suba (Premium)
                                if (risePending && currentPrice >= alert.riseTargetPrice) {
                                                stats.triggered++;
                                                alert.riseTriggered = true;
                                                alert.riseTriggeredAt = Date.now();
                                                alert.riseTriggeredPrice = currentPrice;
                                                changed = true;

                                    if (alert.email) {
                                                      try {
                                                                          await sendRiseEmail(alert, currentPrice);
                                                                          stats.emails++;
                                                      } catch (emailErr) {
                                                                          stats.errors.push({ mlItemId: alert.mlItemId, error: `Email: ${emailErr.message}` });
                                                      }
                    ...
                    if (changed) {
                                  const toKeep = updatedAlerts.filter(a => {
                                                  ...
                                                  const dropActive = !!a.targetPrice && !a.triggered;
                                                  const riseActive = !!a.riseTargetPrice && !a.riseTriggered;
                                                  if (dropActive || riseActive) return true;
                                                  const lastTrigger = Math.max(a.triggeredAt || 0, a.riseTriggeredAt || 0);
                                                  return lastTrigger > 0 && (Date.now() - lastTrigger) < 7 * 24 * 60 * 60 * 1000;
                                  });
                                  if (toKeep.length === 0) {
...
async function sendRiseEmail(alert, currentPrice) {
      const fmt = (p) => `$ ${Math.round(p).toLocaleString('es-AR')}`;
  ...
  <body style="font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',sans-serif;background:#f5f7fa;margin:0;padding:20px;">
  <div style="max-width:560px;margin:0 auto;">
  <div style="background:#e74c3c;border-radius:12px 12px 0 0;padding:20px 24px;">
  <h1 style="color:#fff;font-size:20px;margin:0;">El precio subio &mdash; PrecioML</h1>
  </div>
  <div style="background:#fff;border-radius:0 0 12px 12px;padding:24px;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
  <p style="font-size:15px;color:#222;margin:0 0 16px;">Tenias una alerta de suba activa en:</p>
  <div style="background:#f5f7fa;border-radius:10px;padding:16px;margin-bottom:20px;">
  <p style="font-size:13px;font-weight:600;color:#222;margin:0 0 12px;">${escHtml(alert.title || 'Producto')}</p>
  <table width="100%" cellpadding="0" cellspacing="0">
  <tr>
  <td style="font-size:12px;color:#888;">Precio actual</td>
  <td style="font-size:12px;color:#888;">Tu limite</td>
  </tr>
  <tr>
  <td style="font-size:22px;font-weight:800;color:#e74c3c;padding-top:4px;">${fmt(currentPrice)}</td>
  <td style="font-size:18px;font-weight:600;color:#888;padding-top:4px;">${fmt(alert.riseTargetPrice)}</td>
  </tr>
  </table>
  </div>
  ${alert.url ? `<a href="${escHtml(alert.url)}" style="display:block;background:#3483fa;color:#fff;text-align:center;padding:14px;border-radius:10px;font-size:15px;font-weight:700;text-decoration:none;">Ver publicacion &rarr;</a>` : ''}
  <p style="font-size:12px;color:#aaa;margin:16px 0 0;text-align:center;">Esta alerta ya fue desactivada. Crea una nueva desde la extension si queres seguir avisandote.</p>
  </div>
  <div style="text-align:center;padding:16px;font-size:11px;color:#aaa;">
  Enviado por <a href="https://precioml-backend.vercel.app" style="color:#3483fa;text-decoration:none;">PrecioML</a>
  </div>
  ...
  const response = await fetch(RESEND_API, {
          ...
          body: JSON.stringify({
                    from: 'PrecioML Alertas <alertas@resend.dev>',
                    to: alert.email,
                    subject: `Subio de precio: ${escHtml((alert.title || 'Tu producto').substring(0, 50))} - PrecioML`,
                    html,
          }),
  });
      if (!response.ok) throw new Error(await response.text());
